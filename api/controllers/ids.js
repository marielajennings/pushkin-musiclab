...
module.exports = (rpc, conn, dbWrite) => {
  ...
  router.post('/deleteUser', (req, res, next) => {
    ...
    const responseInput = {
      method: 'raw',
      params: [
        `DELETE FROM "ids_responses" WHERE "user_id" = ${req.body.id}`
      ]
    };
    const stimulusResponseInput = {
      method: 'raw',
      params: [
        `DELETE FROM "ids_stimulusResponses" WHERE "user_id" = ${req
          .body.id}`
      ]
    };
  ...
  router.post('/stimulusResponse', (req, res, next) => {
    ...
    const update = {
      method: 'raw',
      params: [
        `UPDATE "ids_stimuli" SET "num_responses" = "num_responses" + 1 WHERE "stimulus" = '${stimulus}'`
      ]
    };
  ...
  router.post('/clean', (req, res, next) => {
    ...
    if (checkUser(user.name, user.pass)) {
      const clean = {
        method: 'raw',
        params: [
          `DELETE FROM "ids_users" WHERE "id" NOT IN (SELECT "user_id" FROM "ids_responses" UNION SELECT "user_id" FROM "ids_stimulusResponses") AND "created_at" < NOW() - INTERVAL '2 hours'`
        ]
      };
