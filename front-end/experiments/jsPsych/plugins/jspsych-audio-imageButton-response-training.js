/**
 * jspsych-audio-button-response-training
 * Kristin Diep
 *
...
jsPsych.plugins["audio-button-response-training"] = (function() {
	var plugin = {};

	jsPsych.pluginAPI.registerPreload('audio-button-response-training', 'stimulus', 'audio');

	plugin.info = {
		name: 'audio-button-response-training-training',
		description: '',
		parameters: {
			stimulus: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.choices.length; i++) {
      ...
      var str = buttons[i].replace(/%choice%/g, '<img src='+trial.choices[i]+' align='+alignment[i]+' width=300 style="position:relative">');
      html += '<div class="jspsych-audio-button-response-training-button" style="cursor: pointer; display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'" id="jspsych-audio-button-response-training-button-' + i +'" data-choice="'+i+'">'+str+'</div>';
    }
		html += '</div>';
