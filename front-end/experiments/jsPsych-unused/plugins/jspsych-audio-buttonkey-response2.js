...
jsPsych.plugins["audio-button-response"] = (function() {
	...
	plugin.info = {
      ...
      button_html: {
        ...
        default: '<button class="jspsych-btn2"><img src="%choice%" width=80%></button>',
        array: true,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.choices.length; i++) {
      ...
      display_element.querySelector('#jspsych-audio-button-response-button-' + i).addEventListener('click', function(e){
		  var response_key = 'f';
		  var response_button = trial.choices[e.currentTarget.getAttribute('data-choice')];
		  var choice = e.currentTarget.getAttribute('data-choice'); // don't use dataset for jsdom compatibility
        after_response(choice, response_key, response_button);
      });
    }
    ...
    function after_response(choice, response_key, response_button) {
	  ...
	  response.response_button = response_button;
      response.rt = rt;
    ...
    function end_trial() {
      ...
      var trial_data = {
		...
		"response_button": response.response_button
      };
